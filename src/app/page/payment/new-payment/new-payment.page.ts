...
export class NewPaymentPage implements OnInit {
  ...
  ngOnInit() {
    ...
    console.log('Fecha actual formateada (YYYY-MM-DD):', this.dateToday);
    const fechaForm = this.listaFormulario.find(item => item.id === 'fecha');
    if (fechaForm) {
      fechaForm.valueSelect = this.dateToday;
    }
  }
  ...
  clickSegment(value: string) {
    ...
    if (value === 'cuota') {
      this.listaFormulario.push({
        id: 'fechaEnd', titulo: 'Fecha Final', isError: false, placeholder: 'Seleccione la fecha final del gasto', tipo: 'date', required: true, valueSelect: this.dateToday
      })
      this.listaFormulario.push({
        id: 'cuota', titulo: 'Cuotas', isError: false, placeholder: 'Ingrese el numero de cuotas', tipo: 'number', required: false
      })
      this.listaFormulario.push({
        id: 'cuotaNum', titulo: 'Cuotas Pagadas', isError: false, placeholder: 'Ingrese el numero de cuotas pagadas', tipo: 'number', required: false
      })
    } else {
